...
type FlyAndFadeUpParams = {
    y?: number;
    duration?: number;
};

export const flyAndFadeUp = (
    node: Element,
    params: FlyAndFadeUpParams = { y: 16, duration: 150 }
): TransitionConfig => {
    // Fades in and flys upwards relative to the current position
    ...
    return {
        ...
        delay: 0,
        css: (t) => {
            const y = scaleConversion(t, [0, 1], [params.y ?? 16, 0]);
            return styleToString({
                transform: `${transform} translate3d(0, ${y}px, 0)`,
                opacity: t
            });
        },
        easing: cubicOut
