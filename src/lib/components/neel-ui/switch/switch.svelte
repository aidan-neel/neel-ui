<script>
  ...
  <div class="flex items-center">
    <div
      class="switch-container cursor-pointer w-14 h-8 flex items-center border {!enabled ? 'bg-background' : 'bg-white'} rounded-full p-1"
      on:click={toggle}
      bind:this={container}
    >
      <!-- Pill switch toggle -->
      <div
        class={`pill ${enabled ? 'bg-background' : 'bg-white'} border rounded-full shadow-md ${enabled ? 'translate-x-full' : 'translate-x-0'}`}
