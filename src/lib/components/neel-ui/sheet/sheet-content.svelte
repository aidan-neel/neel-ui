<script lang="ts">
    ...
    import PopoverBg from "../modal/popover-bg.svelte";
...
{#if isOpen }
    ...
    <div class="h-screen w-screen shadow-class backdrop-blur-md bg-background bg-opacity-60 z-[98] fixed top-0 left-0"
        ...
        <div on:mouseenter={() => { mouseEntered = true }} on:mouseleave={() => { mouseEntered = false }} transition:fly={{ x: side === "left" ? -80 : 80, duration: 300 }} {...$$restProps} class={cn(className, ` overflow-y-auto flex flex-col p-8 fixed bg-background h-screen w-5/6 md:w-2/4 lg:w-[28rem] z-[99] top-0 ${side === "left" ? 'left-0 border-r' : 'right-0 border-l'} shadow-class`)}>
